...
func RunScanner(ctx context.Context, tasks []models.Repository, numWorkers int, client *github.Client) ([]models.Risks, error) {
	...
	results := make([]models.Risks, 0, len(tasks))
...
func scanJobConsumer(ctx context.Context, scanTaskChan chan scanJob, mu *sync.Mutex, results *[]models.Risks) {
	for {
		select {
		case <-ctx.Done():
			return
		case scanJob, ok := <-scanTaskChan:
			...
			if scanJob.Err != nil {
				mu.Lock()
				*results = append(*results, models.Risks{Repo: scanJob.Repo, Err: scanJob.Err})
				mu.Unlock()
				continue
			}
